...
const MillionaireGame = () => {
  ...
  const handleRevealPhase = async () => {
    ...
    for (const player of players) {
      ...
      if (isCorrect) {
        // Lógica ACUMULATIVA: Soma o valor da pergunta ao saldo
        ...
        if (currentQuestion.isProfessor) {
          ...
          if (room.current_question_index < 6) {
            newValue = Math.max(0, player.current_value - 2000);
          } else if (room.current_question_index < 12) {
            newValue = Math.max(0, player.current_value - 10000);
          } else {
    ...
    setTimeout(async () => {
      ...
      // O jogo só acaba se não houver mais ninguém ativo OU se as perguntas acabarem
  ...
  const handleChoiceClick = (key: string) => {
    ...
    if (currentQuestion.isGreed && key === 'A') {
      ...
      else { showError("Apenas o líder pode ser ganancioso!"); setSelectedChoice(null); }
    }
  };
  ...
  const handleGreedElimination = async (targetId: string) => {
    ...
    await supabase.from('millionaire_players').update({ current_value: target.current_value + myPlayer.current_value }).eq('id', targetId);
    ...
    await supabase.from('millionaire_answers').insert({ room_id: roomId, player_id: myPlayer.id, question_index: room.current_question_index, answer: 'A', is_correct: false });
  };
  ...
  const submitAnswer = async () => {
    ...
    try {
      await supabase.from('millionaire_answers').insert({ room_id: roomId, player_id: myPlayer.id, question_index: room.current_question_index, answer: selectedChoice, is_correct: isCorrect });
      showSuccess("Resposta enviada!");
    } catch (error) { showError("Erro ao enviar resposta."); } finally { setSubmitting(false); }
  };
  ...
  const startGame = async () => {
    ...
    const questionIds = [easy[0].id, easy[1].id, profBonus.id, easy[2].id, easy[3].id, easy[4].id, antibodyBonus.id, medium[0].id, medium[1].id, medium[2].id, medium[3].id, greedTrap.id, medium[4].id, hard[0].id, hard[1].id, hard[2].id, hard[3].id, hard[4].id];
    await supabase.from('millionaire_rooms').update({ status: 'playing', phase: 'question', current_question_index: 0, question_ids: questionIds, question_started_at: new Date().toISOString() }).eq('id', roomId);
  };
  ...
  useEffect(() => {
    ...
    const channel = supabase.channel(`millionaire_realtime_${roomId}`)
      .on('postgres_changes', { event: '*', schema: 'public', table: 'millionaire_rooms', filter: `id=eq.${roomId}` }, (payload) => {
        setRoom(payload.new);
        if (payload.new.phase === 'question') { setSelectedChoice(null); setHiddenOptions([]); setShowTip(false); setDoubleChanceActive(false); setFirstWrongDone(false); setShowGreedSelection(false); }
      })
  ...
  // Correção do bug de carregamento infinito: removemos a trava do myPlayer aqui
  if (loading || !room) {
    return (
  ...
  const canStop = myPlayer && myPlayer.current_value >= 20000 && !myPlayer.is_eliminated && room.phase === 'question' && !myAnswer && !currentQuestion.isProfessor;
  ...
  return (
    <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-8 p-4 animate-in fade-in duration-700">
      <div className="lg:col-span-3 space-y-6 order-2 lg:order-1">
        <Card className="bg-slate-900/80 border-white/10 rounded-3xl overflow-hidden">
          ...
          <div className="p-2 space-y-1">
            {[...PRIZES].reverse().map((prize, idx) => {
              ...
              return (
                <div key={idx} className={cn("flex justify-between px-4 py-1.5 rounded-xl text-[11px] font-black transition-all", isCurrent ? "bg-yellow-600 text-white scale-105 shadow-lg z-10" : isCheckpoint ? "text-emerald-400 bg-emerald-500/5 border border-emerald-500/10" : "text-slate-500")}>
                  <div className="flex items-center gap-2"><span>{levelIdx + 1}</span>{isCheckpoint && <ShieldCheck className="w-3 h-3" />}</div>
        ...
        <Card className="bg-slate-900/80 border-white/10 rounded-3xl overflow-hidden">
          <div className="p-4 bg-white/5 border-b border-white/5 flex items-center justify-between"><h3 className="text-[10px] font-black text-blue-400 uppercase tracking-[0.3em]">Candidatos</h3><Users className="w-3 h-3 text-blue-400" /></div>
              ...
              <div key={p.id} className={cn("flex items-center justify-between p-2 rounded-xl transition-colors", p.is_eliminated ? "bg-red-500/5" : "bg-white/5")}>
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex flex-col">
                    ...
                    {p.is_eliminated && <span className="text-[8px] text-red-500 font-black uppercase">Finalizado</span>}
                  </div>
                </div>
      ...
      <div className="lg:col-span-9 space-y-6 order-1 lg:order-2">
        <div className="flex items-center justify-between bg-white/5 p-4 rounded-3xl border border-white/10 backdrop-blur-xl">
          <div className="flex items-center gap-3"><Trophy className="text-yellow-500" /><span className="font-black text-white tracking-tight">SALA #{room.code}</span></div>
          <div className="flex items-center gap-4">
            {myPlayer?.is_eliminated && room.status === 'playing' && <Badge className="bg-red-600/20 text-red-500 border-red-500/30 flex items-center gap-2 px-3 py-1 animate-pulse"><Ghost className="w-3 h-3" /> MODO ESPECTADOR</Badge>}
            {room.host_id === currentUserId && <Button variant="outline" size="sm" className="border-blue-500/30 text-blue-400 hover:bg-blue-500/10" onClick={() => window.open(`/millionaire/${roomId}/presentation`, '_blank')}><Monitor className="w-4 h-4 mr-2" /> MODO PROJETOR</Button>}
        ...
        {room.phase === 'waiting' ? (
          <Card className="bg-white/5 border-white/10 rounded-[3rem] p-16 text-center space-y-8 backdrop-blur-2xl">
            <Sparkles className="w-16 h-16 text-yellow-500 mx-auto animate-pulse" /><h2 className="text-4xl font-black text-white">Pronto para o Milhão?</h2><p className="text-slate-400 font-medium">Aguardando o Host iniciar a rodada...</p>
          ...
          <div className="space-y-8">
            {!myPlayer?.is_eliminated && myPlayer && (
              <div className="flex flex-wrap justify-center gap-4">
                <Button onClick={use5050} disabled={used5050 || !!myAnswer || currentQuestion.difficulty === 'special'} className={cn("h-14 px-6 rounded-2xl font-black flex items-center gap-2 transition-all", used5050 || currentQuestion.difficulty === 'special' ? "bg-slate-800 text-slate-600 border-white/5" : "bg-violet-600 hover:bg-violet-500 text-white shadow-lg shadow-violet-900/20")}><Split className="w-5 h-5" /> 50/50</Button>
                <Button onClick={useDoubleChance} disabled={usedDouble || !!myAnswer || currentQuestion.difficulty === 'special'} className={cn("h-14 px-6 rounded-2xl font-black flex items-center gap-2 transition-all", usedDouble || currentQuestion.difficulty === 'special' ? "bg-slate-800 text-slate-600 border-white/5" : "bg-blue-600 hover:bg-blue-500 text-white shadow-lg shadow-blue-900/20")}><Repeat className="w-5 h-5" /> DUPLA CHANCE</Button>
                <Button onClick={useTip} disabled={usedTip || !!myAnswer || currentQuestion.difficulty === 'special'} className={cn("h-14 px-6 rounded-2xl font-black flex items-center gap-2 transition-all", usedTip || currentQuestion.difficulty === 'special' ? "bg-slate-800 text-slate-600 border-white/5" : "bg-emerald-600 hover:bg-emerald-500 text-white shadow-lg shadow-emerald-900/20")}><Lightbulb className="w-5 h-5" /> DICA</Button>
                {canStop && <Button onClick={handleStop} className="h-14 px-8 rounded-2xl font-black bg-red-600 hover:bg-red-500 text-white shadow-lg shadow-red-900/20 animate-in zoom-in duration-500"><HandMetal className="mr-2 w-5 h-5" /> PARAR E LEVAR R$ {myPlayer.current_value.toLocaleString()}</Button>}
              </div>
            )}
            {currentQuestion.isAntibody && <div className="bg-blue-500/10 border border-blue-500/20 p-4 rounded-2xl flex items-center gap-3 text-blue-400 animate-pulse"><AlertCircle className="w-5 h-5 shrink-0" /><p className="text-sm font-black uppercase tracking-widest">AVISO: O ÚLTIMO DO RANKING SERÁ ELIMINADO APÓS ESTA PERGUNTA!</p></div>}
            {showTip && <div className="bg-emerald-500/10 border border-emerald-500/20 p-4 rounded-2xl flex items-center gap-3 text-emerald-400 animate-in slide-in-from-top duration-500"><Lightbulb className="w-5 h-5 shrink-0" /><p className="text-sm font-bold italic">"{currentQuestion.tip}"</p></div>}
            <Card className="bg-white/90 border-white/20 rounded-[3rem] p-12 shadow-2xl relative overflow-hidden"><div className="absolute top-0 left-0 w-full h-2 bg-yellow-500" /><div className="absolute top-4 right-8 flex items-center gap-2 bg-orange-600/20 px-4 py-1.5 rounded-full border border-orange-500/30"><Timer className="w-4 h-4 text-orange-600" /><span className="text-orange-600 font-black text-lg">{timeLeft}s</span></div><h2 className="text-3xl md:text-5xl font-black text-slate-950 text-center leading-tight tracking-tight mt-4">{currentQuestion.question}</h2></Card>
            {showGreedSelection ? (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 animate-in zoom-in duration-500">
                {players.filter(p => p.id !== myPlayer?.id && !p.is_eliminated).map(p => (
                  <Button key={p.id} onClick={() => handleGreedElimination(p.id)} className="h-20 rounded-3xl font-black text-xl bg-red-600 hover:bg-red-500 text-white border-2 border-red-400 shadow-lg">
                    <Avatar className="w-10 h-10 mr-3 border border-white/20"><AvatarImage src={p.avatar_url} className="object-cover" /><AvatarFallback className="bg-slate-800 text-xs">{p.name[0]}</AvatarFallback></Avatar>
                    ELIMINAR {p.name.toUpperCase()}
                  </Button>
                ))}
              </div>
            ) : (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {Object.entries(currentQuestion.options).map(([key, val]) => (
                  <Button key={key} disabled={myPlayer?.is_eliminated || !!myAnswer || submitting || hiddenOptions.includes(key)} onClick={() => handleChoiceClick(key)} className={cn("h-20 rounded-3xl font-black text-xl transition-all border-2 text-left justify-start px-8", myAnswer?.answer === key ? "bg-yellow-600 border-yellow-400 text-white shadow-lg" : selectedChoice === key ? "bg-blue-600 border-blue-400 text-white" : hiddenOptions.includes(key) ? "opacity-0 pointer-events-none" : "bg-white/5 border-white/10 text-white hover:bg-white/10", myPlayer?.is_eliminated && "cursor-default opacity-60")}>
                    <span className="w-10 h-10 rounded-xl bg-white/10 flex items-center justify-center mr-4 text-sm">{key}</span>{val}
                  </Button>
                ))}
              </div>
            )}
            {!myAnswer && !myPlayer?.is_eliminated && !showGreedSelection && myPlayer && (
              <div className="flex justify-center"><Button onClick={submitAnswer} disabled={!selectedChoice || submitting} className="h-16 px-12 bg-emerald-600 hover:bg-emerald-500 text-white font-black rounded-2xl text-lg shadow-xl shadow-emerald-900/20">{submitting ? <Loader2 className="animate-spin mr-2" /> : <Send className="mr-2" />}CONFIRMAR RESPOSTA</Button></div>
            )}
            {myAnswer && <div className="text-center animate-pulse"><p className="text-emerald-400 font-black uppercase tracking-widest">Resposta enviada! Aguarde o fim do tempo...</p></div>}
          </div>
        ) : room.phase === 'reveal' ? (
          <div className="space-y-8 animate-in zoom-in duration-500">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <Card className="bg-emerald-500/10 border-emerald-500/20 rounded-3xl p-6">
                <h3 className="text-emerald-400 font-black uppercase text-xs tracking-widest mb-4 flex items-center gap-2"><CheckCircle2 className="w-4 h-4" /> Acertaram</h3>
                <div className="space-y-2">
                  {players.filter(p => p.last_answered_index === room.current_question_index && !p.is_eliminated).map(p => (
                    <div key={p.id} className="text-white font-bold flex justify-between items-center">
                      <div className="flex items-center gap-2"><Avatar className="w-6 h-6"><AvatarImage src={p.avatar_url} className="object-cover" /><AvatarFallback className="text-[8px]">{p.name[0]}</AvatarFallback></Avatar><span>{p.name}</span></div>
              ...
              <Card className="bg-red-500/10 border-red-500/20 rounded-3xl p-6">
                <h3 className="text-red-400 font-black uppercase text-xs tracking-widest mb-4 flex items-center gap-2"><XCircle className="w-4 h-4" /> Eliminados / Pararam</h3>
                <div className="space-y-2">
                  {players.filter(p => p.is_eliminated && p.last_answered_index === room.current_question_index).map(p => (
                    <div key={p.id} className="text-slate-400 font-bold flex justify-between items-center">
                      <div className="flex items-center gap-2"><Avatar className="w-6 h-6 grayscale"><AvatarImage src={p.avatar_url} className="object-cover" /><AvatarFallback className="text-[8px]">{p.name[0]}</AvatarFallback></Avatar><span>{p.name}</span></div>
            ...
            <div className="max-w-md mx-auto space-y-4">
              {players.sort((a, b) => b.current_value - a.current_value).map((p, i) => (
                <div key={p.id} className={cn("flex items-center justify-between p-6 rounded-3xl border", i === 0 ? "bg-yellow-600/20 border-yellow-500" : "bg-white/5 border-white/10")}>
                  <div className="flex items-center gap-4"><span className="font-black text-white text-xl">{i + 1}º</span><Avatar className="w-10 h-10 border border-white/10"><AvatarImage src={p.avatar_url} className="object-cover" /><AvatarFallback className="font-black">{p.name[0]}</AvatarFallback></Avatar><span className="font-black text-white text-xl">{p.name}</span></div>
