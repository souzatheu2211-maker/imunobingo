...
import { Trophy, Timer, Users, Sparkles, Skull, Loader2 } from 'lucide-react';
...
const MillionairePresentation = () => {
  ...
  const currentQuestion = currentQuestionId === 'MALICE_ROUND' 
    ? { id: 'MALICE_ROUND', question: 'RODADA DA MALDADE', is_special: true, special_type: 'malice' } as any
    : MILLIONAIRE_QUESTIONS.find(q => q.id === currentQuestionId) || MILLIONAIRE_QUESTIONS[0];
    ...
    <div className="min-h-screen bg-slate-950 p-12 flex flex-col gap-12 overflow-hidden">
      <div className="flex justify-between items-center">
        <div className="space-y-2">
          ...
          <Badge className="bg-yellow-600/20 text-yellow-500 border-yellow-500/30 px-4 py-1 text-sm font-black">ARENA DE CONHECIMENTO</Badge>
        </div>
        <div className="flex items-center gap-12">
          ...
          {room.phase === 'question' && currentQuestion.id !== 'MALICE_ROUND' && (
            <div className={cn("p-8 rounded-[2.5rem] border-4 flex items-center gap-6", timeLeft <= 5 ? "bg-red-600/20 border-red-500 animate-pulse" : "bg-orange-600/10 border-orange-500/30")}>
      ...
      <div className="grid grid-cols-12 gap-12 flex-1">
        <div className="col-span-8 flex flex-col gap-8">
          {currentQuestion.id === 'MALICE_ROUND' ? (
            <div className="flex flex-col items-center justify-center h-full space-y-8 animate-in zoom-in duration-700">
              <Skull className="w-40 h-40 text-red-500 animate-bounce" />
              <h2 className="text-7xl font-black text-white text-center">RODADA DA MALDADE</h2>
              <p className="text-3xl text-slate-400 font-bold">O líder está decidindo o destino de alguém...</p>
            </div>
          ) : room.phase === 'question' || room.phase === 'reveal' ? (
            <div className="space-y-8 animate-in zoom-in duration-700">
              <Card className="bg-white border-none rounded-[4rem] p-16 shadow-2xl relative overflow-hidden">
                <div className={cn("absolute top-0 left-0 w-full h-4", currentQuestion.is_special ? "bg-red-500" : "bg-yellow-500")} />
                <h2 className="text-5xl md:text-6xl font-black text-slate-950 text-center leading-tight">{currentQuestion.question}</h2>
              </Card>
              <div className="grid grid-cols-2 gap-6">
                {Object.entries(currentQuestion.options || {}).map(([key, val]) => (
                  <div key={key} className={cn("p-8 rounded-[2.5rem] border-4 text-3xl font-black flex items-center gap-6", room.phase === 'reveal' && key === currentQuestion.correct ? "bg-emerald-500 border-emerald-400 text-white scale-105" : "bg-white/5 border-white/10 text-white")}>
            ...
            <div className="flex flex-col items-center justify-center h-full space-y-12 animate-in fade-in duration-1000">
              <Trophy className="w-72 h-72 text-yellow-500" />
              <h2 className="text-8xl font-black text-white tracking-tighter text-center">DESAFIO <br /> <span className="text-yellow-500">CONCLUÍDO</span></h2>
            </div>
          ) : (
          ...
          <Card className="bg-white/5 border-white/10 rounded-[3rem] h-full overflow-hidden backdrop-blur-xl shadow-2xl">
            ...
            <div className="p-8 space-y-4">
              {players.sort((a, b) => b.current_value - a.current_value).map((p, i) => (
                <div key={p.id} className={cn("flex items-center justify-between p-6 rounded-3xl border transition-all duration-500", p.is_eliminated ? "opacity-30 bg-red-500/5 grayscale" : "bg-white/5 border-white/10")}>
                  <div className="flex items-center gap-4">
                    <span className={cn("text-2xl font-black w-8", i === 0 ? "text-yellow-500" : "text-slate-600")}>{i + 1}º</span>
                    <Avatar className="w-12 h-12 border-2 border-white/10"><AvatarFallback className="bg-slate-800 font-black">{p.name[0]}</AvatarFallback></Avatar>
                    <div className="flex flex-col">
                      <span className="text-2xl font-black text-white tracking-tight">{p.name}</span>
                      <span className={cn("text-[10px] font-black uppercase", p.is_eliminated ? "text-red-500" : "text-emerald-500")}>{p.is_eliminated ? "Eliminado" : "Ativo"}</span>
                    </div>
                  </div>
